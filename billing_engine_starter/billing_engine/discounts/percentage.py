...
class PercentageDiscount(Discount):
    def __init__(self, percentage: Decimal) -> None:
        ...
        if isinstance(percentage, float):
            raise TypeError("PercentageDiscount percentage must be a Decimal, not a float")
        if percentage < Decimal("0") or percentage > Decimal("1"):
            raise ValueError("PercentageDiscount percentage must be between 0 and 1")
        self._percentage = percentage

    def apply(self, subtotal: Money, context: DiscountContext) -> Money:
        # TODO Day 1
        return subtotal * self._percentage
